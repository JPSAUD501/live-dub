...
1.  `recent_transcription_fragments`: A list of the latest speech transcription chunks in {config.INPUT_LANGUAGE_NAME_FOR_PROMPT}.
    ...
    *   Your critical task, therefore, is to use `native_speech_history_processed_by_llm` to meticulously identify only the *genuinely new semantic information* in the latest fragments that hasn't been covered by your previous processing. Do not re-translate content that is merely a restatement or slight refinement of what's already in the history unless it introduces significant new meaning.
...
1.  **Identify New, Complete Segment**:
    *   **Step 1: Determine Raw New Text**: Conceptually, concatenate all text from `recent_transcription_fragments`. Then, identify the portion of this concatenated text that comes *after* the content already present in `native_speech_history_processed_by_llm`. This is your "candidate raw new text".
    *   **Step 2: Extract Longest Complete Prefix**: From this "candidate raw new text", find the longest leading portion that forms a coherent thought, sentence, or independent clause. This portion is your potential `newly_transcribed_segment_processed`.
    ...
    *   **Handling Ellipses ('...') in Relation to Speaker Intent**:
        ...
        *   If the "longest leading portion" identified in Step 2 naturally forms a complete thought *before* an ellipsis that indicates the *speaker's thought was cut off*, then your `newly_transcribed_segment_processed` should be that complete thought *without* the trailing ellipsis and the incomplete part.
    ...
    *   **Step 3: Final Decision**: If no such complete segment can be extracted from the "candidate raw new text" (e.g., it's all fragmentary, or only contains content already processed), then `newly_transcribed_segment_processed` should be an empty string for this turn.
